...
$tab1 =[]; # tableau temporaire contenant les élements de la colonne 1 du fichier ct
$tab2 =[]; # tableau temporaire contenant les élements de la colonne 5 du fichier ct
$tab3 =[];# tableau temporaire contenant les bases de la séquence
...
while(my $line = <FOut>)
{
	...
	else
	{
		...
		{	
			...
			$tab{$nomSeq}= $mySequence; # remplissage du tableau associatif avec le nom et le struct
		}
	}
}	
...
foreach $sequence (values %tab)
{
	$element5Copy = -1;
	$nbnucleotides = 0;
	for ($i=0;$i<$sequence->length;$i++) # parcours des deux colonnes 1 et 5
	{
		$element1 =  $sequence->colonne1($i);
		$element5 =  $sequence->colonne5($i);
		if ($element5 == 0) # lorsque l'on rencontre un 0
		{
			$nbnucleotides = $nbnucleotides + 1; #nombre des nucléotides de la boucle
		}
		else # tout ce qui n'est pas 0
		{
			if (($i!=0)&&($sequence->colonne5($i-1) == 0)&&($element1 == $element5Copy)) # ici le premier nombre non nul arpès une suite de 0 et 
			{				
